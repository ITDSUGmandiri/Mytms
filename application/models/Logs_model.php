...
class Logs_model extends CI_Model
{
	...
	*/
	public function get_location_log_by_code($code)
	{
		...
		$this->db->join(
			$this->units_table,
			$this->locations_log_table.'.unit_id = '.$this->units_table.'.id'
		);
		...
		// if code provided
		if ($code!='') {
			$this->db->where($this->locations_log_table.'.aset_id', $code);
		}
	...
	*/
	public function get_status_log_by_code($code)
	{
		...
		// if code provided
		if ($code!='') {
			$this->db->where($this->status_log_table.'.aset_id', $code);
		}
	...
	{
		$this->db->insert($this->unit_log_table, $log);
