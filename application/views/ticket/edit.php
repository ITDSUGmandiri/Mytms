			...
			<div class="box">
				...
				<div class="box-body <?php if (!isset($open_form)){ echo "hide";} ?>" id="add_new">
					<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
						...
						<form action="<?php echo base_url('ticket/save_edit') ?>" method="post" autocomplete="off" class="form form-horizontal" enctype="multipart/form-data">
							...
							<?php foreach($list_ticket->result() as $data){ ?>

							<input type="hidden" name="id_ticket" id="id_ticket" value="<?php echo encrypt_url($data->id); ?>">
								...
								<div class="col-md-8 <?php if (form_error('id_lok')) {echo "has-error";} ?>">
									...
									<select name="id_lok" id="id_lok" class="form-control" required>
										...
										<?php foreach ($data_list_area->result() as $val){ ?>
											<option value="<?php echo $val->id; ?>" <?php echo ($data->id_area == $val->id?'selected':'') ?>><?php echo $val->nama_lokasi; ?> </option> 
										<?php } ?>

									</select>

									<input type="hidden" name="hidden_location" id="hidden_location" value="<?php echo $data->id_area ?>">
									<input type="hidden" name="curr_area_id" id="curr_area_id" value="<?php $data->id_area ?>">
							...
							<div class="form-group">
								...
								<div class="col-md-4">
									...
									<input type="hidden" name="hidden_id_unit" id="hidden_id_unit" value="<?php echo $data->id_unit ?>">
									<input type="hidden" name="curr_unit_id" id="curr_unit_id" value="<?php $data->id_unit ?>">
								...
								<div class="col-md-8 <?php if (form_error('service_family_id')) {echo "has-error";} ?>">
									...
									<select name="service_family_id" id="service_family_id" class="form-control" required>
										...
										<?php foreach ($data_list_service_family->result() as $val){ ?>
											...
											<option value="<?php echo $val->id_service_family; ?>" <?php echo ($data->service_family_id == $val->id_service_family?'selected':'') ?>><?php echo $val->service_family_name; ?> </option> 
										<?php } ?>
							...
							<div class="form-group">
								...
								<div class="col-md-8 <?php if (form_error('type_incident')) {echo "has-error";} ?>">
									...
									<input type="hidden" name="hidden_type_incident_id" id="hidden_type_incident_id" value="<?php echo $data->type_problem ?>">
							...
							<div class="form-group">
								...
								<div class="col-md-8 <?php if (form_error('sub_type_incident')) {echo "has-error";} ?>">
									...
									<input type="hidden" name="hidden_sub_type_incident_id" id="hidden_sub_type_incident_id" value="<?php echo $data->sub_type_incident_id ?>">
							...
							<div class="form-group">
								...
								<div class="col-md-8 <?php if (form_error('petugas1')) {echo "has-error";} ?>">
									...
									<input type="hidden" name="hidden_petugas1_id" id="hidden_petugas1_id" value="<?php echo $data->petugas1_id ?>">
									<input type="hidden" name="hidden_username1" id="hidden_username1" value="<?php echo $data->pic ?>">
							...
							<div class="form-group">
								...
								<div class="col-md-8 <?php if (form_error('petugas2')) {echo "has-error";} ?>">
									...
									<input type="hidden" name="hidden_petugas2_id" id="hidden_petugas2_id" value="<?php echo $data->petugas2_id ?>">
									<input type="hidden" name="hidden_username2" id="hidden_username2" value="<?php echo $data->pic1 ?>">
	...
	$(document).ready(function(){
		...
		$('#type_incident').change(function(){
    		...
    		load_dropdown_sub_type_incident(this.value);
		});
	...
	function load_dropdown_type_incident(service_family_id){
		...
		dataType: 'JSON', 
		data: {service_family_id: document.getElementById("service_family_id").value},
		success: function(data){
			if (data.is_data_ada){
				...
				//iterate over the data and append a select option
				$.each(data.list_data, function (key, val){
					//console.log(hidden_type_incident_id, val);
	...
	function load_dropdown_sub_type_incident(type_incident) {
	...
	//console.log('xxx', type_incident);
    $.ajax({
        url: '<?php echo site_url() . 'ticket/load_dropdown_sub_type_incident'; ?>',
        dataType: 'JSON',
        data: {type_incident_id: type_incident === undefined ? document.getElementById("hidden_type_incident_id").value : type_incident},
        success: function(data) {
